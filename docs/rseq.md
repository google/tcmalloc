...
current virtual CPU ID (read from `__rseq_abi` at `tcmalloc_vcpu_id_offset`, a
thread-local copy of `__rseq_virtual_flat_cpu_id_offset` that lives next to
`__rseq_abi` so that the fast paths reach it through the thread pointer they
have already resolved, rather than through the GOT):
...
"rseq v2" has made that write conditional and terminates a process that writes
the ID fields itself. Validating the region rather than overlapping the cached
pointer with `cpu_id_start` removes both dependencies.
