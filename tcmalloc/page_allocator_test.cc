...
TEST_F(PageAllocatorTest, ShrinkFailureNotRetriedUntilChangeTest) {
  // Turn off subrelease so that we take the ShrinkHardBy path.
  const bool old_subrelease = Parameters::hpaa_subrelease();
  Parameters::set_hpaa_subrelease(false);
  ...
  // Nothing has been freed or released since the shrink failed, so
  // allocations that did not grow the heap must not retry (and fail) again.
  for (int i = 0; i < 16; ++i) {
    PageHeapSpinLockHolder l;
    allocator_.ShrinkToUsageLimit(Length(0), /*may_have_grown=*/false);
  }
  ...
  // Growth always re-checks the limit, and fails again.
  {
    PageHeapSpinLockHolder l;
    allocator_.ShrinkToUsageLimit(Length(0), /*may_have_grown=*/true);
  }
  ...
  // ... after which non-growing allocations are gated again.
  for (int i = 0; i < 16; ++i) {
    PageHeapSpinLockHolder l;
    allocator_.ShrinkToUsageLimit(Length(0), /*may_have_grown=*/false);
  }
  ...
  // Freeing normal1 makes memory releasable (normal2 keeps the hugepage alive,
  // so HugePageFiller::Put does not unback it), so the next non-growing
  // allocation retries exactly once and succeeds.
  Delete(normal1, kSpanInfo, MemoryTag::kNormal);
  for (int i = 0; i < 16; ++i) {
    PageHeapSpinLockHolder l;
    allocator_.ShrinkToUsageLimit(Length(0), /*may_have_grown=*/false);
  }
  EXPECT_EQ(3, allocator_.limit_hits(PageAllocator::kSoft));
  EXPECT_EQ(
      1, allocator_.successful_shrinks_after_limit_hit(PageAllocator::kSoft));
