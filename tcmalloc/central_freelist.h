...
inline Span* CentralFreeList<Forwarder>::ReleaseToSpans(
...
#else
  // Widen to 32 bits so codegen uses native clz/lzcnt instead of the extra
  // shifts/masks in CountLeadingZeroes16.  The bit_width value is unchanged.
  const uint8_t prev_bitwidth =
      absl::bit_width(static_cast<uint32_t>(prev_allocated));
#endif
  ...
  const uint8_t cur_bitwidth =
      absl::bit_width(static_cast<uint32_t>(cur_allocated));
#endif
...
#else
      // Widen to 32 bits so codegen uses native clz/lzcnt instead of the extra
      // shifts/masks in CountLeadingZeroes16.  The bit_width value is
      // unchanged.
      const uint8_t prev_bitwidth =
          absl::bit_width(static_cast<uint32_t>(prev_allocated));
#endif
