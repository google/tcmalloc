...
class TransferCache {
 public:
  ...
  void InsertRange(int size_class, absl::Span<void* absl_nonnull> batch)
    ...
    // miss; issuing it before the freelist's lock acquisition (a full barrier
    // on x86) would force the store to complete before the CAS could execute.
    // Issued afterwards, it drains from the store buffer asynchronously.  These
    // counters are lossy and only sampled by the background resize thread.
 ...
 private:
  ...
  //
  // A pointer-sized FreeList (BackingTransferCache) is an immutable forwarder
  // with no lock, so it needs no isolation and packs into the miss counters'
  // cacheline, keeping the sharded TransferCache at two cachelines.
