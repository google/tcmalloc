...
class TransferCache {
 public:
  ...
  void InsertRange(int size_class, absl::Span<void* absl_nonnull> batch)
    ...
    // Record the miss after the freelist call.  The counters' cacheline is
    // written by every missing core, so issuing the RMW ahead of the
    // freelist's lock acquisition (a full barrier on x86) would serialize the
    // two cache misses; issued afterwards, the store drains asynchronously.
 ...
 private:
  ...
  // A pointer-sized FreeList (BackingTransferCache) has no lock and packs into
  // the miss counters' cacheline instead.
