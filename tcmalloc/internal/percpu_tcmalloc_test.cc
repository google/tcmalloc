...
// remote operations from being observed.  Each thread learns the field when it
// first caches a region; until then it names the dummy region, which no CPU id
// matches.
TEST_F(TcmallocSlabTest, TheCpuIdOffsetNamesTheRegisteredField) {
  ...
  // Take the path a first allocation on a new thread takes: UsePerCpuCache()
  // gates every allocation on IsFastNoInit(), falling back to IsFast() to
  // register a thread that nothing else has, and only then reaches
  // CacheCpuSlab().
  std::thread t([&]() {
    ASSERT_TRUE(IsFast());
    ASSERT_EQ(tcmalloc_slabs, reinterpret_cast<uintptr_t>(tcmalloc_dummy_slab));
    const auto [cpu, cached] = slab_.CacheCpuSlab();
    ASSERT_GE(cpu, 0);
    ASSERT_TRUE(cached);
    EXPECT_NE(tcmalloc_slabs, reinterpret_cast<uintptr_t>(tcmalloc_dummy_slab));
