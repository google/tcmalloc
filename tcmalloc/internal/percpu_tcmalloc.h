...
// Everything the x86 fast paths reach through the thread pointer -- __rseq_abi,
// the cached slab region address that precedes it, and the CPU id within it --
// is addressed off [scratch], which the prologue loads with __rseq_abi's
// distance from the thread pointer.  Naming that distance once, instead of
// letting the compiler materialize it for each "m" operand, leaves the CPU id's
// run time offset without a register of its own.  It would need one for the
// whole block, and the fast paths that return a size, take an alignment, or
// take a hot/cold hint have none to give: they already use every caller-saved
// register, so it costs them a callee-saved register and the push and pop that
// go with it.  For the same reason the distance shares [scratch] rather than
// taking a register of its own: it is dead by TCMALLOC_RSEQ_FETCH_SLABS, which
// overwrites it with the value it addresses.
//
// @GOTTPOFF rather than @TPOFF: We cannot assume local-exec at compile time.
// The linker relaxes the load to an immediate once it turns out to be linking
// an executable.
#define TCMALLOC_RSEQ_FETCH_RSEQ "movq __rseq_abi@GOTTPOFF(%%rip), %[scratch]\n"

#if !defined(__PIC__) && !defined(__PIE__)
#define TCMALLOC_RSEQ_SET_CS(name, tmp) \
  TCMALLOC_RSEQ_FETCH_RSEQ              \
  "movq $__rseq_cs_" #name "_%=, %%fs:8(%[scratch])\n"
#else
#define TCMALLOC_RSEQ_SET_CS(name, tmp)                 \
  TCMALLOC_RSEQ_FETCH_RSEQ                              \
  "mov __rseq_cs_" #name "_%=@GOTPCREL(%%rip), %q[" tmp \
  "]\n"                                                 \
  "movq %q[" tmp "], %%fs:8(%[scratch])\n"
#endif

// Loads tcmalloc_slabs, the cached slab region address, into <dest>.  This is
// the last use of [scratch] as a thread pointer offset, so <dest> may be
// "scratch".
#define TCMALLOC_RSEQ_FETCH_SLABS(dest) \
  "movq %%fs:%c[rseq_slabs_offset](%[scratch]), %[" dest "]\n"

// Loads the current CPU id, a 16 bit value, into <dest>, a 32 bit operand.
// tcmalloc_vcpu_id_offset says where within __rseq_abi the kernel reports it.
#define TCMALLOC_RSEQ_FETCH_CPU(dest)                 \
  "movl %%fs:%c[rseq_vcpu_off](%[scratch]), %k[" dest \
  "]\n"                                               \
  "movzwl %%fs:(%[scratch],%q[" dest "]), %k[" dest "]\n"
...
// Loads the current CPU id, a 16 bit value, into <dest>, a 64 bit register
// operand.  tcmalloc_vcpu_id_offset says where within __rseq_abi the kernel
// reports it; both are reached relative to the sampler address, which is
// already materialized (see TCMALLOC_RSEQ_INPUTS).
#define TCMALLOC_RSEQ_FETCH_CPU(dest)             \
  "ldr %w[" dest                                  \
  "], [%[sampler_addr], #%c[sampler_vcpu_off]]\n" \
  "add %[" dest "], %[sampler_addr], %[" dest     \
  "]\n"                                           \
  "ldrh %w[" dest "], [%[" dest "], #%c[sampler_rseq_offset]]\n"
...
// The prologue assumes there is a [scratch] scratch register operand.  On x86
// it leaves __rseq_abi's distance from the thread pointer there, which the
// critical section must consume before overwriting [scratch]; see
// TCMALLOC_RSEQ_FETCH_SLABS.  <tmp> names a second scratch register operand,
// dead once the prologue returns.
#define TCMALLOC_RSEQ_PROLOGUE(name, tmp)                                     \
  ...
  "_trampoline_PREEMPTION_OR_SIGNAL_BASED_PROFILER_ACTIVE_%=\n"               \
  ".popsection\n"                        /* Prepare */                        \
  "3:\n" TCMALLOC_RSEQ_SET_CS(name, tmp) /* Start */                          \
      "4:\n"
#ifdef __aarch64__
...
#define TCMALLOC_RSEQ_INPUTS                                                 \
  [sampler_addr] "r"(subtle::percpu::GetThreadSamplerAddress()),             \
      ...
      [sampler_rseq_offset] "n"(TCMALLOC_SAMPLER_RSEQ_OFFSET),               \
      [sampler_vcpu_off] "n"(TCMALLOC_SAMPLER_CPU_ID_OFFSET_OFFSET)
#else
#define TCMALLOC_RSEQ_INPUTS                                                 \
  [rseq_vcpu_off] "n"(TCMALLOC_RSEQ_CPU_ID_OFFSET_OFFSET),                   \
...
inline ABSL_ATTRIBUTE_ALWAYS_INLINE bool StoreCurrentCpu(
  ...
  // The calling thread need not have cached a region, so it may not have
  // published the CPU id's location for the asm below yet.
  subtle::percpu::SyncCpuIdOffset();
#endif
#if TCMALLOC_INTERNAL_PERCPU_USE_RSEQ && defined(__x86_64__)
  uint32_t tmp;
  asm(TCMALLOC_RSEQ_PROLOGUE(TcmallocSlab_Internal_StoreCurrentCpu, "tmp")
          R"(
      )" TCMALLOC_RSEQ_FETCH_CPU("tmp") R"(
...
static inline ABSL_ATTRIBUTE_ALWAYS_INLINE bool TcmallocSlab_Internal_Push(
  ...
  uint32_t tmp;
  asm goto(
      TCMALLOC_RSEQ_PROLOGUE(TcmallocSlab_Internal_Push, "tmp")
      // tmp = cpu + TCMALLOC_SLAB_CPU_BIAS;
      TCMALLOC_RSEQ_FETCH_CPU("tmp") "addl $%c[slab_cpu_bias], %[tmp]\n"
      // scratch = tcmalloc_slabs;
      TCMALLOC_RSEQ_FETCH_SLABS("scratch")
...
static inline ABSL_ATTRIBUTE_ALWAYS_INLINE bool TcmallocSlab_Internal_Push(
  ...
  asm goto(
      ...
      //   goto overflow_label;
      TCMALLOC_RSEQ_FETCH_CPU("scratch")
...
inline ABSL_ATTRIBUTE_ALWAYS_INLINE void* TcmallocSlab<NumClasses>::Pop(
  ...
  asm goto(TCMALLOC_RSEQ_PROLOGUE(TcmallocSlab_Internal_Pop, "current")
           // current = cpu + TCMALLOC_SLAB_CPU_BIAS;
           TCMALLOC_RSEQ_FETCH_CPU(
               "current") "addl $%c[slab_cpu_bias], %k[current]\n"
           // scratch = tcmalloc_slabs;
           TCMALLOC_RSEQ_FETCH_SLABS("scratch")
...
inline ABSL_ATTRIBUTE_ALWAYS_INLINE void* TcmallocSlab<NumClasses>::Pop(
  ...
  asm goto(
      TCMALLOC_RSEQ_PROLOGUE(TcmallocSlab_Internal_Pop, "current")
      // region_start = tcmalloc_slabs;
...
std::pair<int, bool> TcmallocSlab<NumClasses>::CacheCpuSlabSlow() {
  ...
  // Synchronize() registered rseq for this thread if nothing else had, which
  // is what settles where the kernel reports the CPU id.  Tell the fast paths
  // before any region address is cached: until then, they compare against the
  // dummy region's marker, which no CPU id read from any offset matches.
  subtle::percpu::SyncCpuIdOffset();
  const auto [slabs, shift] = GetSlabsAndShift(std::memory_order_relaxed);
