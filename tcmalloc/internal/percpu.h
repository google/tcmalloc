...
==== THEIRS //depot/google3/tcmalloc/internal/percpu.h#87
#define TCMALLOC_RSEQ_SLABS_OFFSET -4
...
#define TCMALLOC_CACHED_SLABS_MASK (1ul << TCMALLOC_CACHED_SLABS_BIT)
    == ==
    YOURS  // corpagent-eng-ckennelly:ckennelly_resolve:185604:citc/google3/tcmalloc/internal/percpu.h
#define TCMALLOC_RSEQ_SLABS_OFFSET -8

// Offset from the sampler to the cached slabs address.
...
#define TCMALLOC_INTERNAL_PERCPU_USE_RSEQ 0
        ...
        GOOGLE_MALLOC_SECTION_BEGIN namespace tcmalloc {
  ...
  // marker.  A thread that has not synchronized yet reads kCpuIdUninitialized
  // or kCpuIdUnsupported from that field, which truncate to 0xffff and 0xfffe
  // and so produce these two values.  Neither may name a running region.
  ...
  inline constexpr uint32_t kUnsupportedMarker =
      uint32_t{static_cast<uint16_t>(kCpuIdUnsupported)} +
      TCMALLOC_SLAB_CPU_BIAS;
  ...
  // kUnsupportedMarker (0xffff) is not, so it is the binding constraint: CPU
  // ids stop one short of the one that would produce it.
  ...
  //
  // The validation is off the critical path. Its loads
  // (__rseq_virtual_flat_cpu_id_offset and then the CPU id out of __rseq_abi)
  // form a dependency chain that is disjoint from the one that produces the
  // object (tcmalloc_slabs, then the size class header, then the object
  // pointer). The two chains meet only at a perfectly predicted branch, so
  // out-of-order execution resolves the validation alongside the allocation
  ...
  // read-only memory, both because nothing may write to it and so that it is
  // not mistaken for a real region.
  ...
  // reach it through the GOT.  Hidden visibility lets the fast paths address
  // this copy relative to the program counter instead.
  ...
  // It's not used other than ensuring that our asm file with the real TLS
  // layout is actually linked. Without it linker manages to not link the asm
  // file in some configurations since all symbols it provides are already
  // defined (as weak), even though the asm file is in the same static library
  // as other linked in files.
  ...
  // compare against the slab marker. This is the low half of
  // kernel_rseq::cpu_id when running on real CPUs, and kernel_rseq::vcpu_id or
  ...
  // kernel_rseq::mm_cid when virtual CPUs are in use.
  //
  // This reads tcmalloc_vcpu_id_offset, so that it agrees with the assembly
  // fast paths and costs no indirection through the GOT.
  ...
  // Functions below are implemented in the architecture-specific
  // percpu_rseq_*.S files.
  extern "C" {
  ...
  // choose the right vCPU source based on the initialized mode. Wrapping it
  // into a class helps to restrict access and avoid accidental misuse.
  class VirtualCpu {
   public:
    // Returns the last vCPU ID since the last synchronization point. This may
    // be used where the vCPU ID is not used to derive RSEQ-validated state.
    // Returns kCpuIdUninitialized if this thread has never synchronized with a
    // vCPU ID, or kCpuIdUnsupported if RSEQ is not used.
    //
    ...
    // ID after a thread preemption was detected. This function may be
    // expensive, so it should only be called on slow paths.
    static int Synchronize();

   private:
    // The return value of Synchronize() may be overridden by tests if they
    // define VirtualCpu::TestSynchronize().
