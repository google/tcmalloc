...
// Offset from __rseq_abi to tcmalloc_vcpu_id_offset, the offset within
// __rseq_abi of the CPU id the fast paths read.
#define TCMALLOC_RSEQ_CPU_ID_OFFSET_OFFSET -16
...
// Offset from the sampler to tcmalloc_vcpu_id_offset.
#define TCMALLOC_SAMPLER_CPU_ID_OFFSET_OFFSET \
  (TCMALLOC_SAMPLER_RSEQ_OFFSET + TCMALLOC_RSEQ_CPU_ID_OFFSET_OFFSET)
...
// (tcmalloc_vcpu_id_offset and then the CPU id out of __rseq_abi) form a
// dependency chain that is disjoint from the one that produces the
// object (tcmalloc_slabs, then the size class header, then the object pointer).
// The two chains meet only at a perfectly predicted branch, so out-of-order
// execution resolves the validation alongside the allocation rather than ahead
// of it.
//
...
//
// This is a per-thread copy of __rseq_virtual_flat_cpu_id_offset, which the
// fast paths cannot read cheaply: it is defined in another module, so the
// compiler must reach it through the GOT.  Living next to __rseq_abi, the copy
// is addressable relative to the thread pointer that the fast paths have
// already resolved.  CacheCpuSlabSlow() writes it before it caches a region
// address, and a thread that has cached nothing names tcmalloc_dummy_slab,
// whose marker matches no CPU id whatever offset it was read from.
extern "C" ABSL_CONST_INIT thread_local volatile uint32_t
    tcmalloc_vcpu_id_offset ABSL_ATTRIBUTE_INITIAL_EXEC;
...
ABSL_CONST_INIT thread_local volatile uint32_t tcmalloc_vcpu_id_offset
    ABSL_ATTRIBUTE_WEAK = 0;
...
// Publishes where the kernel reports the CPU id to the assembly paths, which
// read it from tcmalloc_vcpu_id_offset rather than from
// __rseq_virtual_flat_cpu_id_offset.  Push and Pop compare against the cached
// region, and a thread that has cached nothing names tcmalloc_dummy_slab,
// whose marker no CPU id matches; so CacheCpuSlabSlow() calls this before it
// caches a region address.  StoreCurrentCpu() compares against a region its
// caller named, cached or not, so it calls this too.
// REQUIRES: rseq registration has completed; that is what settles the location.
inline void SyncCpuIdOffset() {
