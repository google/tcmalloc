...
static void InitPerCpu() {
  ...
  volatile auto vcpu_id_offset_addr =
      reinterpret_cast<uintptr_t>(&tcmalloc_vcpu_id_offset);
  ...
  // Likewise for the offset of the CPU id within __rseq_abi.
  TC_CHECK_EQ(vcpu_id_offset_addr,
              rseq_abi_addr + TCMALLOC_RSEQ_CPU_ID_OFFSET_OFFSET);
...
#if TCMALLOC_INTERNAL_PERCPU_USE_RSEQ
  // Registration has settled on where the kernel reports the CPU id.  The fast
  // paths read 16 bits there.
...
void FenceCpu(int vcpu) {
  ...
  if (UsingRseqVirtualCpus()) {
    ...
    // an unrelated core.
    //
    // TODO(b/543348098): Implement support for vCPU-centric membarrier.
    FenceAllCpus();
    return;
  }
