...
static_assert(kNormalP0Mask == kNormalMask);
static_assert((kNormalP0Mask & kTagOrBadDeallocationMask) == kNormalP0Mask);
static_assert(kNormalPartitions == 1 ||
              (kNormalP1Mask & kTagOrBadDeallocationMask) == kNormalP1Mask);
static_assert(kNormalP0Mask != kNormalP1Mask);
static_assert(kNormalP0Mask != kColdMask && kNormalP1Mask != kColdMask);
...
// more assert related variables, and additional forced inlinings etc).
  ...
  // The caller derived kPartition from the pointer's tag.  'policy.is_cold()'
  // means the ptr has kCold tag. To make the delete operation correctly maps
  // to the cold size class, partition 0 needs to be used. If partition 1 used,
  // it will be wrongly mapped to NormalP1 classes (see above diagrams).
  // Additional note: partition number specified at allocation time is not
  // recorded in the tag for cold objects and 'PartitionFromPointerFast' will
  // return wrong results for them (and therefore there is assertion failure if
  // used for kCold ptrs)
