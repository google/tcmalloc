...
// (uptr & kTagOrBadDeallocationMask) against one of these selects the
// partition in the same compare that rejects nullptr, misaligned, out-of-range,
// sampled, and cold pointers.
...
// nullptr carries tag 0, so it fails both compares and is handled afterwards.
...
inline ABSL_ATTRIBUTE_ALWAYS_INLINE void do_free_with_size(void* ptr,
  ...
  // single compare that also selects the partition.
...
// more assert related variables, and additional forced inlinings etc).
  ...
  // The caller derived kPartition from the pointer's tag.  Cold objects are
  // tagged kCold regardless of the partition they were allocated from, so they
  // map to partition 0's size classes (see the diagrams above) and
  // PartitionFromPointerFast() is meaningless for them.
