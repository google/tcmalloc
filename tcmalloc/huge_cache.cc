...
template <size_t kEpochs>
void MinMaxTracker<kEpochs>::Extrema::operator+=(const Extrema &other) {
  max += other.max;
  min += other.min;
}
...
void MinMaxTracker<kEpochs>::SeriesStats::Report(size_t offset,
                                                 const Extrema &e) {
  range.Report(NHugePages(offset));
  max.Report(e.max);
  min.Report(e.min);
  sum += e;
...
template <size_t kEpochs>
void MinMaxTracker<kEpochs>::SeriesStats::operator+=(SeriesStats &other) {
  ASSERT(range.min > other.range.max);
...
void MinMaxTracker<kEpochs>::SeriesStats::Print(TCMalloc_Printer *out) const {
  ...
  out->printf("\n%s %4zu /%4zu %9zu %9zu %12.3f %12.3f", "hugepages_max", count,
              range.max.raw_num() + 1, max.min.raw_num(), max.max.raw_num(),
  ...
  out->printf("\n%s  =%8.3f %9zu %9zu %12.3f %12.3f", "hugepages_min",
              count / static_cast<double>(range.max.raw_num() + 1),
              min.min.raw_num(), min.max.raw_num(),
              safe_ratio(sum.min.raw_num(), count),
