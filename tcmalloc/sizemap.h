 ...
 public:
  ...
  [[nodiscard]] ABSL_ATTRIBUTE_NO_SANITIZE_UNDEFINED
    ...
    if (kHasExpandedClasses && policy.is_cold()) {
      TC_ASSERT_LT(
          idx + (policy.partition() + kSecurityPartitions) * kClassArraySize,
          kClassArraySizePartitions);
      size_class =
          class_array_[idx + (policy.partition() + kSecurityPartitions) *
                                 kClassArraySize];
    } else {
      TC_ASSERT_LT(idx + policy.partition() * kClassArraySize,
                   kClassArraySizePartitions);
      size_class = class_array_[idx + policy.partition() * kClassArraySize] +
